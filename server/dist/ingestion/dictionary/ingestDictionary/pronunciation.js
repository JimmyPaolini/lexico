function getClassicalPhonemes(wordString) {
    ...
    for (let i = 0; i < word.length; i++) {
        if (word[i] === 'h') {
            ...
            if (isVowel(i + 1) && (i === 0 || isVowel(i - 1)))
                phonemes.push('J');
            else
                phonemes.push(classicalPhonemes[word[i]]);
        }
        else if (word[i] === 'j') {
    ...
    for (let i = 0; i < word.length; i++) {
        if (word[i] === 'c') {
            if (i + 1 < word.length && ['e', 'i', 'y'].includes(word[i + 1])
                || i + 2 < word.length && ['ae', 'oe'].includes(word[i + 1] + word[i + 2]))
                phonemes.push('ch');
            ...
            if (i - 2 >= 0 && i + 1 < word.length && wordString.slice(i - 2, i + 2) === 'mihi'
                || i - 2 >= 0 && i + 2 < word.length && wordString.slice(i - 2, i + 3) === 'nihil')
                phonemes.push('k');
        }
        else if (word[i] === 'i') {
            if (i === 0 && i + 1 < word.length && isVowel(word[i + 1]))
                phonemes.push('j');
            else if (i - 1 > 0 && i + 1 < word.length && isVowel(word[i - 1]) && isVowel(word[i + 1]))
                phonemes.push('j');
            else
...
function phonemesToPronunciations(phonemes) {
    ...
    function buildPronunciations(prev, next) {
        ...
        if (Array.isArray(phoneme))
            ...
            buildPronunciations([...prev, phoneme], [...next]);
    }
...
function getPronunciations(word, style) {
    if (style === 'classical')
        return getClassicalPronunciations(word);
    else if (style === 'ecclesiastical')
        return getEcclesiasticalPronunciations(word)[0];
    else if (style === 'vulgar')
        return [];
    else
        throw new Error(`unknown pronunciation style`);
}
