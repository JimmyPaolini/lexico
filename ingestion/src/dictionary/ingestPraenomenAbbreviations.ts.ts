...
export default async function ingestPraenomenAbbreviations() {
  ...
  for (const abbreviation of Object.keys(praenomenAbbreviations)) {
    ...
    if (praenomen.masculine && !praenomen.feminine) {
      ;(entry.inflection as NounInflection).gender = "masculine"
    } else if (!praenomen.masculine && praenomen.feminine) {
      ;(entry.inflection as NounInflection).gender = "feminine"
    } else {
      ;(entry.inflection as NounInflection).gender = "neuter"
    }
