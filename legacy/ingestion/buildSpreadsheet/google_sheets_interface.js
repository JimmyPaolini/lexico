...
function entriesToRows(entries) {
    ...
    for (const entry of entries) {
        for (const etymology of entry.etymologies) {
            ...
            if (Array.isArray(etymology.forms)) row.push(etymology.forms.map(form =>
                form ? Object.values(form).filter(v => v).join(' ') : '').join('; '));
            else if (etymology.forms) row.push(formsFlatten(etymology.forms).join(', '));
            else row.push('');
...
function getFirstLetter(word) {
    const [l1, l2, ..._] = [...word.toLowerCase()];
    if (!l1.match(/[a-z-]/) && !l1.match(/[a-z-]/)) return '*';
    if (l1 === '-') return l2;
    else return l1;
}

module.exports = { appendRowsToSpreadsheet, entriesToRows, getFirstLetter, clearTable, sortTable };