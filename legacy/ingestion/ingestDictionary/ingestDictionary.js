...
const {
  getFirstLetter,
} = require("../buildSpreadsheet/google_sheets_interface")

try {
  return ingestDictionary(process.argv[2], process.argv[3])
} catch (e) {
  return emailMe("Error Ingesting Dictionary", e.toString())
}
...
async function ingestDictionary(firstLetter = "a", lastLetter = "z") {
  ...
  for (let fileName of files) {
    const entry = require(path.join(
      process.cwd(),
      `../dictionary/html/${fileName}`,
    ))
