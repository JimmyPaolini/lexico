...
export function romanToDecimal(roman: string) {
  ...
  for (let i = 0; i < roman.length; i++) {
    ...
    if (i + 1 < roman.length && v1 < value(roman[i + 1]))
      decimal -= v1;
    else
      decimal += v1;
  }
...
export function decimalToRoman(decimal: number) {
  ...
  function convertDigit(digit: number, low: string, mid: string, top: string) {
    if (digit < 4)
      roman += new Array(digit).fill(low).join("");
    else if (digit === 4)
      roman += low + mid;
    else if (digit < 9)
      roman += mid + new Array(digit - 5).fill(low).join("");
    else if (digit === 9)
      roman += low + top;
  }
}

export function romanNumeralize(str: string) {
  if (!str)
    return "";
